...
fn test_data_dir_path() {
    ...
    // ChannelState, by default, is configured for Agenterm.
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(data_dir(), home_dir.join(".agenterm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(data_dir(), home_dir.join(".local/share/agenterm"));
        } else if #[cfg(windows)] {
            assert_eq!(data_dir(), home_dir.join("AppData\\Roaming\\agenterm\\Agenterm\\data"));
        } else {
...
fn test_config_local_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(config_local_dir(), home_dir.join(".agenterm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(config_local_dir(), home_dir.join(".config/agenterm"));
        } else if #[cfg(windows)] {
            assert_eq!(config_local_dir(), home_dir.join("AppData\\Local\\agenterm\\Agenterm\\config"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
#[test]
fn test_gui_app_id_maps_agenterm_tui_to_agenterm_gui() {
    let gui_app_id =
        gui_app_id_for_channel(Channel::Oss, AppId::new("dev", "agenterm", "AgentermTui"));

    assert_eq!(gui_app_id.to_string(), "dev.agenterm.Agenterm");
}
...
fn test_gui_config_and_mcp_paths_resolve_explicit_sources() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(gui_config_dir, home_dir.join(".agenterm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(gui_config_dir, home_dir.join(".config/agenterm"));
        } else if #[cfg(windows)] {
            assert_eq!(
                gui_config_dir,
                home_dir.join("AppData\\Local\\agenterm\\Agenterm\\config")
            );
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
...
fn test_warp_home_config_dir_path() {
    ...
    let expected_dir_name = match ChannelState::data_profile() {
        Some(data_profile) => format!(".agenterm-{data_profile}"),
        None => ".agenterm".to_string(),
    };
...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/dev.agenterm.Agenterm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(cache_dir(), home_dir.join(".cache/agenterm"));
        } else if #[cfg(windows)] {
            assert_eq!(cache_dir(), home_dir.join("AppData\\Local\\agenterm\\Agenterm\\cache"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Agenterm.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/dev.agenterm.Agenterm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(state_dir(), home_dir.join(".local/state/agenterm"));
        } else if #[cfg(windows)] {
            assert_eq!(state_dir(), home_dir.join("AppData\\Local\\agenterm\\Agenterm\\data"));
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
...
#[test]
fn test_project_path_for_agenterm_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("dev", "agenterm", "Agenterm"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.agenterm.Agenterm");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(project_dirs.project_path(), "agenterm");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "agenterm\\Agenterm");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
