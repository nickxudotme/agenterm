...
impl Sender {
    ...
    pub fn start_file(&mut self, info: FileInfo) -> Result<(), Error> {
        ...
        self.acknowledged_position = 0;
        self.sent_position = 0;
        self.pending_event = Some(SenderEvent::FileStart);
    ...
    pub fn submit_file(&mut self, data: &[u8]) -> Result<(), Error> {
        ...
        self.sent_position = self.sent_position.max(next_offset);
        self.frame_needs_header = false;
    ...
    ///
    /// Re-queues the request for the current acknowledgement phase. File data
    /// resumes from the highest position the receiver has confirmed.
    ///
    /// # Errors
    ///
    /// * [`Backpressure`](crate::Error::Backpressure) when outgoing bytes are still pending
    pub fn timeout(&mut self) -> Result<(), Error> {
        ...
        match self.state {
            ...
            SenderPhase::WaitSinitAck => self.queue_zsinit()?,
            SenderPhase::WaitFilePos => self.queue_zfile()?,
            SenderPhase::WaitFileAck => self.on_zrpos(self.acknowledged_position)?,
            SenderPhase::WaitFileDone => self.queue_zeof(self.sent_position)?,
            SenderPhase::WaitFinish => self.queue_zfin()?,
            SenderPhase::ReadyForFile | SenderPhase::NeedFileData | SenderPhase::Done => {}
        }
        Ok(())
    }
    ...
    pub fn poll(&mut self) -> Action<'_> {
        if let Some(event) = self.pending_event.take() {
            return Action::Event(match event {
                SenderEvent::FileStart => Event::FileStarted(FileInfo::new(
                    &self.file_name,
                    Some(Position::new(self.file_size)),
                )),
                SenderEvent::FileSkip => Event::FileSkipped,
    ...
    /// Highest valid position confirmed by the receiver for the current file.
    #[must_use]
    pub const fn acknowledged_position(&self) -> Position {
        Position::new(self.acknowledged_position)
    }
    ...
    fn handle_header(&mut self, header: Header) -> Result<(), Error> {
        match header.frame() {
            ...
            Frame::ZACK if self.state == SenderPhase::WaitSinitAck => self.on_sinit_ack(),
            Frame::ZRPOS => self.on_zrpos(header.count()),
            Frame::ZACK if self.state == SenderPhase::WaitFileAck => self.on_zrpos(header.count()),
            Frame::ZSKIP => {
                self.on_zskip();
                Ok(())
            }
    ...
    fn on_zrinit(&mut self, header: Header) -> Result<(), Error> {
        ...
        match self.state {
            SenderPhase::WaitReceiverInit => {
                ...
                if flags[3] & Zrinit::ESCCTL.bits() == 0 {
                    self.queue_zsinit()?;
                    self.state = SenderPhase::WaitSinitAck;
                } else {
                    self.on_receiver_ready()?;
                }
            }
    ...
    fn on_sinit_ack(&mut self) -> Result<(), Error> {
        self.on_receiver_ready()
    }

    fn on_receiver_ready(&mut self) -> Result<(), Error> {
        if self.has_file {
            self.queue_zfile()?;
            self.state = SenderPhase::WaitFilePos;
        } else {
    ...
    fn on_zrpos(&mut self, offset: u32) -> Result<(), Error> {
        match self.state {
            ...
            SenderPhase::WaitFilePos | SenderPhase::WaitFileAck | SenderPhase::NeedFileData => {
                if offset > self.file_size || offset > self.sent_position {
                    return Err(Error::InvalidState);
                }
                self.acknowledged_position = self.acknowledged_position.max(offset);
    ...
    fn on_zskip(&mut self) {
        if matches!(
            ...
            self.pending_request = None;
            self.frame_remaining = 0;
            self.pending_event = Some(SenderEvent::FileSkip);
            self.state = SenderPhase::ReadyForFile;
        }
    }
