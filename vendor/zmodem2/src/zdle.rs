...
pub const ZDLE_TABLE: [u8; 0x100] = [
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x4a, 0x0b, 0x0c, 0x4d, 0x0e, 0x0f,
...
/// Returns the wire representation for a byte when all control characters are escaped.
pub const fn escape_control(byte: u8) -> u8 {
    if byte & 0x60 == 0 {
        byte ^ 0x40
    } else {
        ZDLE_TABLE[byte as usize]
    }
}
