...
impl Receiver {
    ...
    pub fn submit_wire(&mut self, input: &[u8]) -> Result<usize, Error> {
        ...
        loop {
            ...
            if self.state == ReceiverPhase::WaitOo && !self.oo_header {
                ...
                if byte == crate::ZPAD && self.oo_count == 0 {
                    self.oo_header = true;
                    let mut prefix = SliceReader::new(&[crate::ZPAD]);
                    self.header_reader.read(&mut prefix)?;
                } else if self.oo_count != 0 || !matches!(byte, 0x11 | 0x91) {
        ...
        if let Some(event) = self.pop_event() {
            return Action::Event(match event {
                ReceiverEvent::FileStart => Event::FileStarted(FileInfo::new(
                    &self.file_name,
                    self.file_size_known.then_some(Position::new(self.file_size)),
                )),
    ...
    fn blocked(&self) -> bool {
        self.outgoing() || !self.drain_file().is_empty() || self.pending_event_len != 0
            || matches!(self.state, ReceiverPhase::FileAcceptPending | ReceiverPhase::SessionEnd)
    }
    ...
    fn handle_header(&mut self, header: Header) -> Result<(), Error> {
        match header.frame() {
            ...
            {
                self.data_encoding = if header.encoding() == Encoding::ZHEX {
                    Encoding::ZBIN
                } else {
                    header.encoding()
                };
            ...
            Frame::ZEOF
                if (self.state == ReceiverPhase::FileWaitingSubpacket
                    ...
                    && self.file_active && header.count() == self.count =>
            {
            ...
            Frame::ZFIN
                if matches!(
                    self.state,
                    ReceiverPhase::SessionBegin | ReceiverPhase::FileBegin | ReceiverPhase::WaitOo
                ) && !self.file_active =>
            {
                self.queue_zfin()?;
                self.state = ReceiverPhase::WaitOo;
                self.oo_header = false;
                self.oo_count = 0;
            }
            _ => {}
        }
    ...
    fn parse_zfile_buf(&mut self) -> Result<(), Error> {
        ...
        if let Some(size_str_bytes) = fields.next() {
            ...
            self.file_size_known = !size_field_bytes.is_empty();
        } else {
            self.file_size = 0;
            self.file_size_known = false;
        }
